...
void if_three(t_push *push , int i)
{
    ...
    while ((iz_check_topb(push, i, '3', '0')) || (iz_check_topb(push, i, '2', '2')))
    {
        cmd_pa(push);
...
void if_two(t_push *push, int i)
{
    ...
    while ((iz_check_topb(push, i, '1', '2')))
        cmd_rb(push, 1);
    while ((iz_check_botb(push, i, '1', '2')) || (iz_check_botb(push, i, '1', '3')))
        cmd_rrb(push, 1);
    while ((iz_check_topb(push, i, '1', '2')) || (iz_check_topb(push, i, '1', '3')))
        cmd_pa(push);
}

void if_one_helper(t_push *push, int i)
{
    while((iz_check_botb(push, i, '1', '0')) || (iz_check_botb(push, i, '0', '3')))
    {
...
void if_one(t_push *push, int i)
{
    while (1)
    {
        ...
        {
            cmd_pa(push);
            if ((iz_check_topb(push, i, '1', '0')) || (iz_check_topb(push, i, '0', '3')))
               cmd_rr(push);
            else
               cmd_ra(push, 1);             
        }
    ...
    while ((iz_check_topb(push, i, '1', '0')) || (iz_check_topb(push, i, '0', '3')))
        cmd_rb(push, 1);
